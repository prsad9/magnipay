        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
          {rewards.map((reward, i) => (
            ...
            >
              ...
              {/* Content */}
              <h3 className="text-sm font-semibold mb-1.5 leading-snug line-clamp-2">{reward.title}</h3>
              <p className="text-xs text-muted-foreground leading-snug line-clamp-2 flex-grow">{reward.desc}</p>
            </motion.div>
          ))}
        </div>
      </div>
